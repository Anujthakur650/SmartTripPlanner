...
private extension MapView {
    var mainContent: some View {
        VStack(spacing: 0) {
            mapSection
                .frame(height: 320)
                .overlay(alignment: .top, content: { infoOverlay })
            Divider()
            ScrollView {
                scrollContent
            }
    ...
    private var scrollContent: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            if let selectedPlace = viewModel.selectedPlace {
                PlaceDetailCard(viewModel: viewModel, place: selectedPlace) { place in
                    editingPlace = place
                }
            }
            routesSection
...
private extension MapView {
    ...
    func associationEditor(for place: Place) -> AssociationEditorView {
